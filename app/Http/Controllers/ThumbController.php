...
class ThumbController extends Controller
{
    ...
    public function index($query, $file)
    {
        ...
        if ($extension == 'webp') $file = str_replace('.webp', '', $file);
        ...
        if ($imageOptimize) {
            if(@exec('echo EXEC') == 'EXEC') {
                $imageControl->optimize();
            }
        }
        
        if ($extension == 'webp') $imageControl->format(Manipulations::FORMAT_WEBP);
